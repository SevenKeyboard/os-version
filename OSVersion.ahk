...
class OSVersion
{
    ...
    isWindowsServer()    {
        ...
        osvi:=new this.OSVERSIONINFOEXW
        ...
        return !dllCall("Kernel32.dll\VerifyVersionInfoW", "Ptr",osvi.ptr, "UInt",VER_PRODUCT_TYPE, "UInt64",dwlConditionMask)
    }
    HIBYTE(w)    { ;  // #define HIBYTE(w) ((BYTE)((((DWORD_PTR)()) >> 8) & 0xff))
        return ((w>>8)&0xFF)
    }
    LOBYTE(w)    { ;  // #define LOBYTE(w) ((BYTE)(((DWORD_PTR)()) & 0xff))
        return (w&0xFF)
    }
    WIN    {
        get  {
    ...
    WINVER    {
        get  {
            return format("0x{:04X}",(this._lpVersionInformation.dwMajorVersion<<8)|this._lpVersionInformation.dwMinorVersion)
        }
    }
    OSVersionInfoSize    {
        get  {
            return this._lpVersionInformation.dwOSVersionInfoSize
        }
    }
    MajorVersion    {
        get  {
            return this._lpVersionInformation.dwMajorVersion
        }
    }
    MinorVersion    {
        get  {
            return this._lpVersionInformation.dwMinorVersion
        }
    }
    BuildNumber    {
        get  {
            return this._lpVersionInformation.dwBuildNumber
        }
    }
    PlatformId    {
        get  {
            return this._lpVersionInformation.dwPlatformId
        }
    }
    CSDVersion    {
        get  {
            return this._lpVersionInformation.szCSDVersion
        }
    }
    ServicePackMajor    {
        get  {
            return this._lpVersionInformation.wServicePackMajor
        }
    }
    ServicePackMinor    {
        get  {
            return this._lpVersionInformation.wServicePackMinor
        }
    }
    SuiteMask    {
        get  {
            return this._lpVersionInformation.wSuiteMask
        }
    }
    ProductType    {
        get  {
            return this._lpVersionInformation.wProductType
        }
    }
    Reserved    {
        get  {
            return this._lpVersionInformation.wReserved
        }
    }
    class OSVERSIONINFOEXW
    {
        aMembers:={dwOSVersionInfoSize:{offset:0, type:"UInt"}
        ...
        __new()    {
            this.setCapacity("structure",284), dllCall("Ntdll.dll\RtlFillMemory", "Ptr",this.ptr:=this.getAddress("structure"), "UPtr",284, "Int",0)
            this.dwOSVersionInfoSize:=284
            return this
        }
        __get(aName)    {
            if (aName=="szCSDVersion")
                return strGet(this.ptr+this.aMembers.szCSDVersion.offset,, "UTF-16")
            else if (this.aMembers.hasKey(aName))
                return numGet(this.ptr, this.aMembers[aName].offset, this.aMembers[aName].type)
            return ""
        }
        __set(aName, aValue)
        {
            if (aName!=="aMembers")    {
                if (aName=="szCSDVersion")    {
                    if (127<(len:=strLen(aValue)))
                        aValue:=subStr(aValue,1,len:=127)
                    strPut(aValue, this.ptr+this.aMembers.szCSDVersion.offset, len+1, "UTF-16")
                    return aValue
                }  else if (this.aMembers.hasKey(aName))    {
                    numPut(aValue, this.ptr, this.aMembers[aName].offset, this.aMembers[aName].type)
                    return aValue
                }
            }
        }
